...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.IsFREZApplyTransaction() {
		...
		if err := core.ValidateFREZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
	...
	if tx.IsFREXApplyTransaction() {
		copyState := pool.currentState(ctx).Copy()
		if err := core.ValidateFREXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
