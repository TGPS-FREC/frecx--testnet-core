...
import (
	...
	blockSignerContract "github.com/FRECNET/contracts/blocksigner"
	multiSignWalletContract "github.com/FRECNET/contracts/multisigwallet"
	randomizeContract "github.com/FRECNET/contracts/randomize"
	settingContract "github.com/FRECNET/contracts/setting"
	validatorContract "github.com/FRECNET/contracts/validator"
...
func (w *wizard) makeGenesis() {
	...
	fmt.Println()
	fmt.Println("Which consensus engine to use? (default = S2PoS)")
	fmt.Println(" 1. Ethash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")
	fmt.Println(" 3. S2PoS - solid-state-proof-of-stake")
	...
	case choice == "" || choice == "3":
		genesis.Difficulty = big.NewInt(1)
		genesis.Config.S2PoS = &params.S2PoSConfig{
			Period: 15,
			Epoch:  30000,
			Reward: 0,
		}
		...
		genesis.Config.S2PoS.Period = uint64(w.readDefaultInt(2))

		fmt.Println()
		fmt.Println("How many Ethers should be rewarded to masternode? (default = 2625)")
		reward := uint64(w.readDefaultInt(2625))
		genesis.Config.S2PoS.Reward = reward
		...
		fmt.Println("signers:::signers:::", signers)
		...
		validatorCap.SetString("1000000000000000000000000", 10) // 1 million
		...
		fmt.Println()
		fmt.Println("How many blocks per epoch? (default = 4200)")
		epochNumber := uint64(w.readDefaultInt(4200))
		genesis.Config.S2PoS.Epoch = epochNumber
		genesis.Config.S2PoS.RewardCheckpoint = epochNumber

		fmt.Println()
		fmt.Println("How many blocks before checkpoint need to prepare new set of masternodes? (default = 2100)")
		genesis.Config.S2PoS.Gap = uint64(w.readDefaultInt(2100))

		fmt.Println()
		fmt.Println("What is foundation wallet address? (default = fre0000000000000000000000000000000000000068)")
		...
		fmt.Println("transactOpts:::", transactOpts)
		fmt.Println("contractBackend:::", contractBackend)
		fmt.Println("signers:::", signers)
		fmt.Println("validatorCaps:::", validatorCaps)
		...
		settingAddress, _, err := settingContract.DeploySetting(transactOpts, contractBackend, common.MaxMasternodes, epochNumber, reward, common.TIPIncreaseMasternodes.Uint64())
		contractBackend.Commit()
		fmt.Println("settingAddress:::", settingAddress)
		code, _ = contractBackend.CodeAt(ctx, settingAddress, nil)
		storage = make(map[common.Hash]common.Hash)
		contractBackend.ForEachStorageAt(ctx, settingAddress, nil, f)
		sBalance := big.NewInt(0) // 16m
		genesis.Alloc[common.HexToAddress(common.SettingAddr)] = core.GenesisAccount{
			Balance: sBalance,
			Code:    code,
			Storage: storage,
		}
		...
		contractBackend.Commit()
		fmt.Println("multiSignWalletAddr:::", multiSignWalletAddr)
		...
		contractBackend.Commit()
		fmt.Println("blockSignerAddress:::", blockSignerAddress)
		...
		fmt.Println("big.NewInt(0):::", big.NewInt(0))
		...
		contractBackend.Commit()
		fmt.Println("randomizeAddress:::", randomizeAddress)
		...
		fmt.Println("common.HexToAddress(common.RandomizeSMC)", common.HexToAddress(common.RandomizeSMC))
		fmt.Println("randomizeAddress:::big.NewInt(0):::", big.NewInt(0))
		fmt.Println("Code:::", code)
		fmt.Println("Storage:::", storage)
		...
		contractBackend.Commit()
		fmt.Println("multiSignWalletTeamAddr:::", multiSignWalletTeamAddr)
		...
		fmt.Println("multiSignWalletTeamAddr:::big.NewInt(0):::", balance)
		fmt.Println("Code:::", code)
		fmt.Println("Storage:::", storage)

		fmt.Println()
		fmt.Println("What is swap wallet address for fund 55m FRE?")
