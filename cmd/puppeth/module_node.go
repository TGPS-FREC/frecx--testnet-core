...
{{end}}
RUN \
  echo 'FRE --cache 512 init /genesis.json' > FRE.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ethereum/keystore/ && cp /signer.json /root/.ethereum/keystore/' >> FRE.sh && \{{end}}
	echo $'FRE --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> FRE.sh

ENTRYPOINT ["/bin/sh", "FRE.sh"]
`
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.keyJSON != "" {
			// S2PoS solid-state-proof-of-stake signer
			var key struct {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 FRE --exec admin.nodeInfo.id attach", network, kind)); err != nil {
