...
const (
	clientIdentifier = "FRE" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the FRECNET command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.FREXEnabledFlag,
		utils.FREXDataDirFlag,
		utils.FREXDBEngineFlag,
		utils.FREXDBConnectionUrlFlag,
		utils.FREXDBReplicaSetNameFlag,
		utils.FREXDBNameFlag,
...
func init() {
	// Initialize the CLI app and start FRE
	app.Action = FRE
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright (c) 2018 FRECNET"
...
// FRE is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func FRE(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, cfg FREConfig) {
	...
	if _, ok := ethereum.Engine().(*S2PoS.S2PoS); ok {
		go func() {
			started := false
			ok := false
			slaveMode := ctx.GlobalIsSet(utils.FRESlaveModeFlag.Name)
			var err error
			if common.IsTestnet {
				ok, err = ethereum.ValidateMasternodeTestnet()
				fmt.Println("ValidateMasterNodeTest")
				if err != nil {
					utils.Fatalf("Can't verify masternode permission: %v", err)
				}
			} else {
				fmt.Println("ValidateMasterNode")
