...
func loadConfig(file string, cfg *FREConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, FREConfig) {
	// Load defaults.
	cfg := FREConfig{
		Eth:         eth.DefaultConfig,
		Shh:         whisper.DefaultConfig,
		FREX:        FREx.DefaultConfig,
	...
	if ctx.GlobalBool(utils.FRETestnetFlag.Name) {
		...
		common.TIPTRC21Fee = common.TIPFREXTestnet
		common.TIPTRC21Fee = common.TIPTRC21FeeTestnet
		common.TIPFREXCancellationFee = common.TIPFREXCancellationFeeTestnet
	}
	...
	utils.SetFREXConfig(ctx, &cfg.FREX, cfg.Node.DataDir)
	return stack, cfg
}
...
func makeFullNode(ctx *cli.Context) (*node.Node, FREConfig) {
	stack, cfg := makeConfigNode(ctx)

	// Register FREX's OrderBook service if requested.
	// enable in default
	utils.RegisterFREXService(stack, &cfg.FREX)
