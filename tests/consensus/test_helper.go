...
var (
	...
	acc1Addr    = crypto.PubkeyToAddress(acc1Key.PublicKey)  //fre703c4b2bD70c169f5717101CaeE543299Fc946C7
	acc2Addr    = crypto.PubkeyToAddress(acc2Key.PublicKey)  //fre0D3ab14BBaD3D99F4203bd7a11aCB94882050E7e
	acc3Addr    = crypto.PubkeyToAddress(acc3Key.PublicKey)  //fre71562b71999873DB5b286dF957af199Ec94617F7
	voterAddr   = crypto.PubkeyToAddress(voterKey.PublicKey) //fre5F74529C0338546f82389402a01c31fB52c6f434
	chainID     = int64(1337)
)
...
func getCommonBackend(t *testing.T, chainConfig *params.ChainConfig) *backends.SimulatedBackend {
	...
	contractBackendForSC := backends.NewFRESimulatedBackend(core.GenesisAlloc{
	...
	validatorSCAddr, _, _, err := contractValidator.DeployFREValidator(
		transactOpts,
	...
	contractBackend2 := backends.NewFRESimulatedBackend(core.GenesisAlloc{
...
func GetCandidateFromCurrentSmartContract(backend bind.ContractBackend, t *testing.T) masterNodes {
	...
	validator, err := contractValidator.NewFREValidator(addr, backend)
	if err != nil {
		t.Fatal(err)
	}
...
func PrepareFRETestBlockChain(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig) (*BlockChain, *backends.SimulatedBackend, *types.Block) {
	// Preparation
	var err error
...
func insertBlock(blockchain *BlockChain, blockNum int, blockCoinBase string, parentBlock *types.Block, root string, difficulty int64) (*types.Block, error) {
	block, err := createS2PoSTestBlock(
		blockchain,
...
func createS2PoSTestBlock(bc *BlockChain, parentHash, coinbase string, number int, txs []*types.Transaction, receiptHash string, root common.Hash, difficulty int64) (*types.Block, error) {
...
// 	}
// 	nonce := uint64(0)
// 	to := common.HexToAddress("fre35658f7b2a9e7701e65e7a654659eb1c481d1dc5")
// 	tx := types.NewTransaction(nonce, to, amount, gasLimit, gasPrice, data)
