...
import (
	...
	"github.com/FRECNET/crypto/sha3"
...
func getLendingNonce(userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "FREx_getLendingOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func TestSendLending(t *testing.T) {
	...
	for true {
		...
		// lendToken: USD, collateral: FRE
		// amount 1000 USD
		testSendLending(key, nonce, USDAddress, common.Address{}, new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, USDAddress, common.HexToAddress(common.FRENativeAddress), new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: BTC, collateral: FRE
		// amount 1 BTC
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, BTCAddress, common.HexToAddress(common.FRENativeAddress), new(big.Int).Mul(_1E18, big.NewInt(1)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		// lendToken: FRE, collateral: BTC
		// amount 1000 FRE
		testSendLending(key, nonce, common.HexToAddress(common.FRENativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.FRENativeAddress), BTCAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: FRE, collateral: ETH
		// amount 1000 FRE
		testSendLending(key, nonce, common.HexToAddress(common.FRENativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.FRENativeAddress), ETHAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
	}
}
