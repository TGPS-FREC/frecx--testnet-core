...
func run(evm *EVM, contract *Contract, input []byte, readOnly bool) ([]byte, error) {
	if contract.CodeAddr != nil {
		...
		if p := precompiles[*contract.CodeAddr]; p != nil {
			switch p.(type) {
			case *FRExEpochPrice:
				p.(*FRExEpochPrice).SetTradingState(evm.tradingStateDB)
			case *FRExLastPrice:
				p.(*FRExLastPrice).SetTradingState(evm.tradingStateDB)
			}
			return RunPrecompiledContract(p, input, contract)
		}
	}
	if evm.ChainConfig().IsTIPFREXCancellationFee(evm.BlockNumber) {
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	// above we revert to the snapshot and consume any gas remaining. Additionally
	// when we're in Homestead this also counts for code storage gas errors.
	ret, err = run(evm, contract, input, evm.ChainConfig().IsTIPFREXCancellationFee(evm.BlockNumber))
	if err != nil {
		evm.StateDB.RevertToSnapshot(snapshot)
		if err != ErrExecutionReverted {
			contract.UseGas(contract.Gas)
