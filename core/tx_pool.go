...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	*/
	...
	if tx.IsFREZApplyTransaction() {
		...
		return ValidateFREZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}

	// validate balance slot, token decimal for FREX
	if tx.IsFREXApplyTransaction() {
		copyState := pool.currentState.Copy()
		return ValidateFREXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}
	return nil
}
