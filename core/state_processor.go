...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, tradingState *tradingstate.TradingStateDB, cfg vm.Config, balanceFee map[common.Address]*big.Int) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if tx.IsFREZApplyTransaction() {
			...
			if err := ValidateFREZApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
		...
		if tx.IsFREXApplyTransaction() {
			copyState := statedb.Copy()
			if err := ValidateFREXApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				return nil, nil, 0, err
			}
		}
...
func ApplyTransaction(config *params.ChainConfig, tokensFee map[common.Address]*big.Int, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, FRExState *tradingstate.TradingStateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error, bool) {
	...
	if tx.To() != nil && tx.To().String() == common.TradingStateAddr && config.IsTIPFREX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	if tx.To() != nil && tx.To().String() == common.FREXLendingAddress && config.IsTIPFREX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	if tx.IsTradingTransaction() && config.IsTIPFREX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}

	if tx.IsLendingFinalizedTradeTransaction() && config.IsTIPFREX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	...
	vmenv := vm.NewEVM(context, statedb, FRExState, config, cfg)
	...
	if header.Number.Cmp(maxBlockNumber) <= 0 {
		...
		if addr, ok := blockMap[currentBlockNumber]; ok {
			if strings.ToLower(addr) == strings.ToLower(addrFrom) {
				...
				log.Info("address", addr, "with_balance", bal, "FRE")
