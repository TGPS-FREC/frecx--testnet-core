...
func getNonce(t *testing.T, userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "FREx_getOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func testSendOrderFREUSD(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func testSendOrderFREBTC(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func TestFilled(t *testing.T) {
	...
	//testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(1000000000000000000), big.NewInt(5000)), BTCUSDPrice, "BUY", "NEW", 0)
	//ETH/BTC
	...
	FREBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	time.Sleep(2 * time.Second)
	testSendOrderFREBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderFREBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderFREBTC(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), FREBTCPrice, "SELL", "NEW", 0)

	FREUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(6)) // 0.6
	time.Sleep(2 * time.Second)
	testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), FREUSDPrice, "SELL", "NEW", 0)

}

func TestX10Filled(t *testing.T) {
	FREUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(60)) // 6
	time.Sleep(2 * time.Second)
	testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderFREUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREUSDPrice, "BUY", "NEW", 0)
...
func TestCancelOrder(t *testing.T) {
	FREBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREBTCPrice, "BUY", "NEW", 0)
	time.Sleep(5 * time.Second)
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), FREBTCPrice, "BUY", "CANCELLED", 3)
	time.Sleep(5 * time.Second)
