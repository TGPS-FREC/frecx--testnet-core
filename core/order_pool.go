...
type OrderPool struct {
	...
	pendingState      *tradingstate.FREXManagedState // Pending state tracking virtual nonces
...
func NewOrderPool(chainconfig *params.ChainConfig, chain blockChainFREx) *OrderPool {
...
func (pool *OrderPool) State() *tradingstate.FREXManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	cloneFREXStateDb := pool.currentOrderState.Copy()
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			...
			baseDecimal, err := FREXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.BaseToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get baseDecimal. err: %v", err)
			}
			quoteDecimal, err := FREXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.QuoteToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get quoteDecimal. err: %v", err)
			}
			if err := tradingstate.VerifyBalance(cloneStateDb, cloneFREXStateDb, tx, baseDecimal, quoteDecimal); err != nil {
				return err
			}
		}
	...
	if !tx.IsCancelledOrder() {
		...
		originOrder := cloneFREXStateDb.GetOrder(tradingstate.GetTradingOrderBookHash(tx.BaseToken(), tx.QuoteToken()), common.BigToHash(new(big.Int).SetUint64(tx.OrderID())))
