...
var (
	...
	skipNonceDestinationAddress = map[string]bool{
		common.FREXAddr:                         true,
		common.TradingStateAddr:                 true,
		common.FREXLendingAddress:               true,
		common.FREXLendingFinalizedTradeAddress: true,
	}
)
...
func (tx *Transaction) IsTradingTransaction() bool {
	...
	if tx.To().String() != common.FREXAddr {
		return false
	}
...
func (tx *Transaction) IsLendingTransaction() bool {
	...
	if tx.To().String() != common.FREXLendingAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsLendingFinalizedTradeTransaction() bool {
	...
	if tx.To().String() != common.FREXLendingFinalizedTradeAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsFREXApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}

	addr := common.FREXListingSMC
	if common.IsTestnet {
		addr = common.FREXListingSMCTestNet
	}
	...
	if method != common.FREXApplyMethod {
		return false
	}
...
func (tx *Transaction) IsFREZApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}
	...
	method := common.ToHex(tx.Data()[0:4])
	if method != common.FREZApplyMethod {
		return false
	}
