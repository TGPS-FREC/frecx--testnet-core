...
// blockChain_FREx add order state
...
func (pool *LendingPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPFREX(pool.chain.CurrentBlock().Number()) || pool.chain.Config().S2PoS == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().S2PoS.Epoch {
		return
	}
...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	S2PoSEngine, ok := pool.chain.Engine().(*S2PoS.S2PoS)
	if !ok {
		return ErrNotS2PoS
	}
	FREXServ := S2PoSEngine.GetFREXService()
	lendingServ := S2PoSEngine.GetLendingService()
	if FREXServ == nil {
		return fmt.Errorf("FREx not found in order validation")
	}
	lendingTokenDecimal, err := FREXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := FREXServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	// lendTokenFREPrice: price of lendingToken in FRE quote
	var lendTokenFREPrice, collateralPrice, collateralTokenDecimal *big.Int
	if collateralToken.String() != lendingstate.EmptyAddress {
		collateralTokenDecimal, err = FREXServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
			return fmt.Errorf("validateOrder: failed to get collateralTokenDecimal. err: %v", err)
		}
		lendTokenFREPrice, collateralPrice, err = lendingServ.GetCollateralPrices(pool.chain.CurrentHeader(), pool.chain, cloneStateDb, cloneTradingStateDb, collateralToken, tx.LendingToken())
		if err != nil {
			return err
		}
		if lendTokenFREPrice == nil || lendTokenFREPrice.Sign() <= 0 || collateralPrice == nil || collateralPrice.Sign() <= 0 {
			log.Debug("ValidateLending: ErrInvalidCollateralPrice", "lendTokenFREPrice", lendTokenFREPrice, "collateralPrice", collateralPrice)
			return lendingstate.ErrInvalidCollateralPrice
		}
	}
	if lendTokenFREPrice == nil || lendTokenFREPrice.Sign() == 0 {
		if tx.LendingToken().String() == common.FRENativeAddress {
			lendTokenFREPrice = common.BasePrice
		} else {
			lendTokenFREPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.HexToAddress(common.FRENativeAddress))
			if err != nil {
				return err
			}
		}
	}
	isFREXLendingFork := pool.chain.Config().IsTIPFREXLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isFREXLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
...
func (pool *LendingPool) addTx(tx *types.LendingTransaction, local bool) error {
	if !pool.chainconfig.IsTIPFREX(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
