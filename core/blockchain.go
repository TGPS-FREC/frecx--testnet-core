...
func NewBlockChainEx(db ethdb.Database, FRExDb ethdb.FRExDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addFRExDb(FRExDb)
	}
...
func (bc *BlockChain) addFRExDb(FRExDb ethdb.FRExDatabase) {
	bc.FRExDb = FRExDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		engine, ok := bc.Engine().(*S2PoS.S2PoS)
		if ok {
			tradingService := engine.GetFREXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPFREX(currentBlock.Number()) && bc.chainConfig.S2PoS != nil && currentBlock.NumberU64() > bc.chainConfig.S2PoS.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*S2PoS.S2PoS)
	if ok {
		FREXService := engine.GetFREXService()
		if bc.Config().IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch && FREXService != nil {
			...
			if err == nil {
				return FRExState, nil
			} else {
				return nil, err
			}
		} else {
			FRExState, err := FREXService.GetEmptyTradingState()
			if err == nil {
				return FRExState, nil
			} else {
				return nil, err
	...
	return nil, errors.New("Get FREx state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetFREXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPFREX((*head).Number()) && bc.chainConfig.S2PoS != nil && (*head).NumberU64() > bc.chainConfig.S2PoS.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) insert(block *types.Block) {
	...
	if bc.chainConfig.S2PoS != nil && !bc.chainConfig.IsTIPSigning(block.Number()) {
		engine, ok := bc.Engine().(*S2PoS.S2PoS)
		if ok {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	engine, _ := bc.Engine().(*S2PoS.S2PoS)
	if bc.Config().IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch {
		tradingService := engine.GetFREXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPFREX(bc.CurrentBlock().Number()) && bc.chainConfig.S2PoS != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.S2PoS.Epoch && engine != nil {
			tradingService = engine.GetFREXService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPFREX(recent.Number()) && bc.chainConfig.S2PoS != nil && recent.NumberU64() > bc.chainConfig.S2PoS.Epoch && engine != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService utils.LendingService
	if bc.Config().IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch && engine != nil {
		tradingService = engine.GetFREXService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
	...
	// but due to below code has diverged significantly between ETH and FRE, and current issue we have,
	...
	if status == CanonStatTy {
		...
		if bc.chainConfig.S2PoS != nil && ((block.NumberU64() % bc.chainConfig.S2PoS.Epoch) == (bc.chainConfig.S2PoS.Epoch - bc.chainConfig.S2PoS.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
	...
	for i, block := range chain {
		...
		if bc.Config().IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch {
			...
			if tradingService != nil && lendingService != nil {
				...
				if (block.NumberU64() % bc.chainConfig.S2PoS.Epoch) == 0 {
					if err := tradingService.UpdateMediumPriceBeforeEpoch(block.NumberU64()/bc.chainConfig.S2PoS.Epoch, tradingState, statedb); err != nil {
						return i, events, coalescedLogs, err
					}
				} else {
					...
					if block.Number().Uint64()%bc.chainConfig.S2PoS.Epoch == common.LiquidateLendingTradeBlock {
				...
				if tradingState != nil && tradingService != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid FREx trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("FREX Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("FREX Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPFREX(block.Number()) && bc.chainConfig.S2PoS != nil && block.NumberU64() > bc.chainConfig.S2PoS.Epoch {
		...
		if bc.chainConfig.S2PoS != nil {
			// epoch block
			if (chain[i].NumberU64() % bc.chainConfig.S2PoS.Epoch) == 0 {
				CheckpointCh <- 1
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	for i := len(newChain) - 1; i >= 0; i-- {
		...
		// prepare set of masternodes for the next epoch
		if bc.chainConfig.S2PoS != nil && ((newChain[i].NumberU64() % bc.chainConfig.S2PoS.Epoch) == (bc.chainConfig.S2PoS.Epoch - bc.chainConfig.S2PoS.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
				log.Crit("Error when update masternodes set. Stopping node", "err", err)
			}
	...
	if bc.chainConfig.IsTIPFREX(commonBlock.Number()) && bc.chainConfig.S2PoS != nil && commonBlock.NumberU64() > bc.chainConfig.S2PoS.Epoch {
...
func (bc *BlockChain) UpdateM1() error {
	...
	setAddr := common.HexToAddress(common.SettingAddr)

	fmt.Println("updateM1::addr::", addr)
	fmt.Println("updateM1::addr::", setAddr)

	validator, err := contractValidator.NewFREValidator(addr, client)
	setting, err1 := contractSetting.NewSetting(setAddr, client)

	fmt.Println("updateM1::validator::", validator)
	fmt.Println("updateM1::setting::", setting)
	fmt.Println("updateM1::err1::", err1)
	...
	var tipincreasevalue uint64
	// get candidates from slot of stateDB
	// if can't get anything, request from contracts
	stateDB, err := bc.State()
	if err != nil {
		candidates, err = validator.GetCandidates(opts)
		tipincreasevalue, err1 = setting.GetTIPIncrease(opts)
		fmt.Println("blockchain.go:::ifloop:::", candidates)
		fmt.Println("blockchain.go:::ifloop:tipincreasevalue::", tipincreasevalue)
		if err != nil {
			return err
		}
		if err1 != nil {
		...
		tipincreasevalue = common.TIPIncreaseMasternodes.Uint64()
		fmt.Println("blockchain.go:::else loop:::", candidates)
		fmt.Println("blockchain.go::else :tipincreasevalue:::", tipincreasevalue)
	}
	...
	for _, candidate := range candidates {
		...
		if candidate.String() != "fre0000000000000000000000000000000000000000" {
	...
	if len(ms) == 0 {
		...
		if isEqualTo(big.NewInt(0).SetUint64(tipincreasevalue), header.Number) {
			fmt.Println("isEqualto:::")
			maxMasternodeTest, _err1 := setting.GetMaxMasterNode(opts)
			maxMasternodes = int(maxMasternodeTest)
			fmt.Println("maxMasternodes:::", maxMasternodeTest, maxMasternodes, _err1)
			if err1 != nil {
				return err
			}
		} else {
			// using old masterndoes
			fmt.Println("isNotEqualto:::")
			maxMasternodes = common.MaxMasternodes
			fmt.Println("maxMasternodes:::", maxMasternodes)
		}
		fmt.Println("len(Ms)", len(ms))
		fmt.Println("maxMasternodes", maxMasternodes)
...
func isEqualTo(s, head *big.Int) bool {
	if s == nil || head == nil {
		return false
	}
	return s.Cmp(head) <= 0
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	FREXService := engine.GetFREXService()
	if FREXService == nil || !FREXService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := FREXService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsTradingTransaction() {
			...
			if err := FREXService.RollbackReorgTxMatch(deletedTx.Hash()); err != nil {
