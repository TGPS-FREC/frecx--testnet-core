...
func (v *BlockValidator) ValidateTradingOrder(statedb *state.StateDB, FRExStatedb *tradingstate.TradingStateDB, txMatchBatch tradingstate.TxMatchBatch, coinbase common.Address, header *types.Header) error {
	S2PoSEngine, ok := v.bc.Engine().(*S2PoS.S2PoS)
	if S2PoSEngine == nil || !ok {
		return ErrNotS2PoS
	}
	FREXService := S2PoSEngine.GetFREXService()
	if FREXService == nil {
		return fmt.Errorf("FREx not found")
	}
	...
	for _, txMatch := range txMatchBatch.Data {
		...
		newTrades, newRejectedOrders, err := FREXService.ApplyOrder(header, coinbase, v.bc, statedb, FRExStatedb, tradingstate.GetTradingOrderBookHash(order.BaseToken, order.QuoteToken), order)
		if err != nil {
			return err
		}
	...
	if FREXService.IsSDKNode() {
		v.bc.AddMatchingResult(txMatchBatch.TxHash, tradingResult)
	}
	return nil
}

func (v *BlockValidator) ValidateLendingOrder(statedb *state.StateDB, lendingStateDb *lendingstate.LendingStateDB, FRExStatedb *tradingstate.TradingStateDB, batch lendingstate.TxLendingBatch, coinbase common.Address, header *types.Header) error {
	S2PoSEngine, ok := v.bc.Engine().(*S2PoS.S2PoS)
	if S2PoSEngine == nil || !ok {
		return ErrNotS2PoS
	}
	...
	lendingService := S2PoSEngine.GetLendingService()
	...
	for _, l := range batch.Data {
		...
		// process Matching Engine
		newTrades, newRejectedOrders, err := lendingService.ApplyOrder(header, coinbase, v.bc, statedb, lendingStateDb, FRExStatedb, lendingstate.GetLendingOrderBookHash(l.LendingToken, l.Term), l)
		if err != nil {
			return err
		}
