...
func (FREx *FREX) CommitOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	FRExSnap := tradingStateDB.Snapshot()
	dbSnap := statedb.Snapshot()
	trades, rejects, err := FREx.ApplyOrder(header, coinbase, chain, statedb, tradingStateDB, orderBook, order)
	if err != nil {
		tradingStateDB.RevertToSnapshot(FRExSnap)
...
func (FREx *FREX) ApplyOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
	...
	if order.Status == tradingstate.OrderStatusCancelled {
		err, reject := FREx.ProcessCancelOrder(header, tradingStateDB, statedb, chain, coinbase, orderBook, order)
		if err != nil || reject {
	...
	if orderType == tradingstate.Market {
		...
		trades, rejects, err = FREx.processMarketOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
		...
		trades, rejects, err = FREx.processLimitOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
...
func (FREx *FREX) processMarketOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = FREx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
		...
		log.Debug("processMarketOrder ", "side", side, "bestPrice", bestPrice, "quantityToTrade", quantityToTrade, "volume", volume)
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = FREx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (FREx *FREX) processLimitOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
		newTrades  []map[string]string
		rejects    []*tradingstate.OrderItem
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(minPrice) >= 0 && minPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = FREx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, minPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(maxPrice) <= 0 && maxPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = FREx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, maxPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (FREx *FREX) processOrderList(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, side string, orderBook common.Hash, price *big.Int, quantityStillToTrade *big.Int, order *tradingstate.OrderItem) (*big.Int, []map[string]string, []*tradingstate.OrderItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		if oldestOrder.QuoteToken.String() != common.FRENativeAddress {
			quotePrice = tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(oldestOrder.QuoteToken, common.HexToAddress(common.FRENativeAddress)))
			log.Debug("TryGet quotePrice QuoteToken/FRE", "quotePrice", quotePrice)
			if quotePrice == nil || quotePrice.Sign() == 0 {
				inversePrice := tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.FRENativeAddress), oldestOrder.QuoteToken))
				quoteTokenDecimal, err := FREx.GetTokenDecimal(chain, statedb, oldestOrder.QuoteToken)
				if err != nil || quoteTokenDecimal.Sign() == 0 {
					return nil, nil, nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", oldestOrder.QuoteToken.String(), err)
				}
				log.Debug("TryGet inversePrice FRE/QuoteToken", "inversePrice", inversePrice)
				if inversePrice != nil && inversePrice.Sign() > 0 {
					quotePrice = new(big.Int).Mul(common.BasePrice, quoteTokenDecimal)
					quotePrice = new(big.Int).Div(quotePrice, inversePrice)
					log.Debug("TryGet quotePrice after get inversePrice FRE/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteTokenDecimal)
				}
			}
		} else {
			quotePrice = common.BasePrice
		}
		tradedQuantity, rejectMaker, settleBalanceResult, err := FREx.getTradeQuantity(quotePrice, coinbase, chain, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (FREx *FREX) getTradeQuantity(quotePrice *big.Int, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, takerOrder *tradingstate.OrderItem, makerOrder *tradingstate.OrderItem, quantityToTrade *big.Int) (*big.Int, bool, *tradingstate.SettleBalance, error) {
	baseTokenDecimal, err := FREx.GetTokenDecimal(chain, statedb, makerOrder.BaseToken)
	...
	quoteTokenDecimal, err := FREx.GetTokenDecimal(chain, statedb, makerOrder.QuoteToken)
	if err != nil || quoteTokenDecimal.Sign() == 0 {
		return tradingstate.Zero, false, nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", makerOrder.QuoteToken.String(), err)
	}
	if makerOrder.QuoteToken.String() == common.FRENativeAddress {
...
func GetTradeQuantity(takerSide string, takerFeeRate *big.Int, takerBalance *big.Int, makerPrice *big.Int, makerFeeRate *big.Int, makerBalance *big.Int, baseTokenDecimal *big.Int, quantityToTrade *big.Int) (*big.Int, bool) {
	if takerSide == tradingstate.Bid {
		...
		takerFee = big.NewInt(0).Div(takerFee, common.FREXBaseFee)
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.FREXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.FREXBaseFee, takerFeeRate))
		...
		makerFee = new(big.Int).Div(makerFee, common.FREXBaseFee)
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.FREXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.FREXBaseFee, makerFeeRate))
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, makerPrice)
			log.Debug("Reject order maker , not enough balance ", "makerBalance", makerBalance, " makerOutTotal", makerOutTotal)
...
func DoSettleBalance(coinbase common.Address, takerOrder, makerOrder *tradingstate.OrderItem, settleBalance *tradingstate.SettleBalance, statedb *state.StateDB) error {
	...
	//log.Debug("ApplyFREXMatchedTransaction settle fee for relayers",
	//	"takerRelayerOwner", takerExOwner,
	//	"takerFeeToken", quoteToken, "takerFee", settleBalanceResult[takerAddr][FREx.Fee].(*big.Int),
	//	"makerRelayerOwner", makerExOwner,
	//	"makerFeeToken", quoteToken, "makerFee", settleBalanceResult[makerAddr][FREx.Fee].(*big.Int))
	// takerFee
...
func (FREx *FREX) ProcessCancelOrder(header *types.Header, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB, chain consensus.ChainContext, coinbase common.Address, orderBook common.Hash, order *tradingstate.OrderItem) (error, bool) {
	...
	baseTokenDecimal, err := FREx.GetTokenDecimal(chain, statedb, order.BaseToken)
	if err != nil || baseTokenDecimal.Sign() == 0 {
		log.Debug("Fail to get tokenDecimal ", "Token", order.BaseToken.String(), "err", err)
		return err, false
	}
	// order: basic order information (includes orderId, orderHash, baseToken, quoteToken) which user send to FREx to cancel order
	...
	tokenCancelFee, tokenPriceInFRE := common.Big0, common.Big0
	if !chain.Config().IsTIPFREXCancellationFee(header.Number) {
		tokenCancelFee = getCancelFeeV1(baseTokenDecimal, feeRate, &originOrder)
	} else {
		tokenCancelFee, tokenPriceInFRE = FREx.getCancelFee(chain, statedb, tradingStateDB, &originOrder, feeRate)
	}
	...
	// relayers pay FRE for masternode
	...
	extraData, _ := json.Marshal(struct {
		CancelFee       string
		TokenPriceInFRE string
	}{
		CancelFee:       tokenCancelFee.Text(10),
		TokenPriceInFRE: tokenPriceInFRE.Text(10),
	})
...
// deprecated after hardfork at TIPFREXCancellationFee
func getCancelFeeV1(baseTokenDecimal *big.Int, feeRate *big.Int, order *tradingstate.OrderItem) *big.Int {
	...
	if order.Side == tradingstate.Ask {
		...
		cancelFee = new(big.Int).Div(cancelFee, common.FREXBaseCancelFee)
	} else {
		// BUY 1 BTC => FRE with Price : 10000
...
// return tokenQuantity, tokenPriceInFRE
func (FREx *FREX) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *tradingstate.OrderItem, feeRate *big.Int) (*big.Int, *big.Int) {
	if feeRate == nil || feeRate.Sign() == 0 {
		return common.Big0, common.Big0
	}
	cancelFee := big.NewInt(0)
	tokenPriceInFRE := big.NewInt(0)
	var err error
	if order.Side == tradingstate.Ask {
		cancelFee, tokenPriceInFRE, err = FREx.ConvertFREToToken(chain, statedb, tradingStateDb, order.BaseToken, common.RelayerCancelFee)
	} else {
		cancelFee, tokenPriceInFRE, err = FREx.ConvertFREToToken(chain, statedb, tradingStateDb, order.QuoteToken, common.RelayerCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
	return cancelFee, tokenPriceInFRE
}

func (FREx *FREX) UpdateMediumPriceBeforeEpoch(epochNumber uint64, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB) error {
	...
	if FREx.IsSDKNode() {
		if err := FREx.LogEpochPrice(epochNumber, epochPriceResult); err != nil {
...
// orderbook hash genereted from baseToken, quoteToken at FRECNET/FREx/tradingstate/common.go:214
func (FREx *FREX) LogEpochPrice(epochNumber uint64, epochPriceResult map[common.Hash]*big.Int) error {
	db := FREx.GetMongoDB()
	db.InitBulk()
