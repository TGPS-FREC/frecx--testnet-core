...
func Test_getCancelFee(t *testing.T) {
	...
	FREx.SetTokenDecimal(testTokenA, common.BasePrice)
	// tokenB has decimal 10^8
	tokenBDecimal := new(big.Int).Exp(big.NewInt(10), big.NewInt(8), nil)
	FREx.SetTokenDecimal(testTokenB, tokenBDecimal)

	// set tokenAPrice = 1 FRE
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(testTokenA, common.HexToAddress(common.FRENativeAddress)), common.BasePrice)
	// set tokenBPrice = 1 FRE
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.FRENativeAddress), testTokenB), tokenBDecimal)
	...
	tests := []struct {
		...
		// zero fee test: SELL
		{
			"TokenA/FRE zero fee test: SELL",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  testTokenA,
					QuoteToken: common.HexToAddress(common.FRENativeAddress),
					Quantity:   new(big.Int).SetUint64(10000),
					Side:       tradingstate.Ask,
				},
			},
		...
		// test getCancelFee: SELL
		{
			"TokenA/FRE test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.FRENativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"TokenA/FRE test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
		...
		// test getCancelFee: SELL
		{
			"FRE/TokenA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.FRENativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"FRE/TokenA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := FREx.getCancelFee(nil, nil, tradingStateDb, tt.args.order, tt.args.feeRate); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("getCancelFee() = %v, quantity %v", got, tt.want)
			}
		})
	}

	// testcase: can't get price of token in FRE
	testTokenC := common.HexToAddress("0x1200000000000000000000000000000000000004")
	FREx.SetTokenDecimal(testTokenC, big.NewInt(1))
	...
	if fee, _ := FREx.getCancelFee(nil, nil, tradingStateDb, tokenCOrder.order, tokenCOrder.feeRate); fee != nil && fee.Sign() != 0 {
	...
	FREx.SetTokenDecimal(testTokenD, big.NewInt(0))
	...
	if fee, _ := FREx.getCancelFee(nil, nil, tradingStateDb, tokenDOrder.order, tokenDOrder.feeRate); fee != nil && fee.Sign() != 0 {
		t.Errorf("getCancelFee() = %v, want %v", fee, common.Big0)
	}
