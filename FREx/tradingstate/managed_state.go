...
type FREXManagedState struct {
	*TradingStateDB
	mu        sync.RWMutex
	exchanges map[common.Hash]*exchanges
}

// FREXManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedb *TradingStateDB) *FREXManagedState {
	return &FREXManagedState{
...
func (ms *FREXManagedState) SetState(statedb *TradingStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *FREXManagedState) RemoveNonce(addr common.Hash, n uint64) {
...
func (ms *FREXManagedState) NewNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *FREXManagedState) GetNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if ms.hasAccount(addr) {
		account := ms.getAccount(addr)
...
func (ms *FREXManagedState) SetNonce(addr common.Hash, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *FREXManagedState) HasAccount(addr common.Hash) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *FREXManagedState) getAccount(addr common.Hash) *exchanges {
