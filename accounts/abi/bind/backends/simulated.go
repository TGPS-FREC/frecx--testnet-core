...
// FRE simulated backend for testing purpose.
func NewFRESimulatedBackend(alloc core.GenesisAlloc, gasLimit uint64, chainConfig *params.ChainConfig) *SimulatedBackend {
	...
	consensus := S2PoS.NewFaker(database, chainConfig)

	// Attach mock trading and lending service
	var DefaultConfig = FREx.Config{
		DataDir: "",
	}
	FREXServ := FREx.New(&DefaultConfig)
	lendingServ := FRExlending.New(FREXServ)

	consensus.GetFREXService = func() utils.TradingService {
		return FREXServ
	}
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call FRECNET.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call FRECNET.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call FRECNET.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call FRECNET.CallMsg, block *types.Block, statedb *state.StateDB) (ret []byte, usedGas uint64, failed bool, err error) {
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query FRECNET.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query FRECNET.FilterQuery, ch chan<- types.Log) (FRECNET.Subscription, error) {
