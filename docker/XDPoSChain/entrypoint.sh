...
params=""
accountsCount=$(
  FRE account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    89 )
      ...
      params="$params --apothem --gcmode archive --rpcapi db,eth,net,web3,debug,S2PoS"
      ;;
    90 )
...
# data dir
if [[ ! -d $DATA_DIR/FRE ]]; then
  echo "No blockchain data, creating genesis block."
  FRE init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    FRE  account import ./private_key \
      --datadir $DATA_DIR \
...
if [[ ! -z $DEBUG_MODE ]]; then
  params="$params --gcmode archive --rpcapi db,eth,net,web3,debug,S2PoS"
fi
