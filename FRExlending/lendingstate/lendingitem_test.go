	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := VerifyBalance(true,
				...
				EtherToWei(big.NewInt(1)),
				EtherToWei(big.NewInt(2)),    // FRE price: 0.5 USD => USD/FRE = 2
...
func sendOrder(nonce uint64) {
	...
	err = rpcClient.Call(&result, "FREx_sendLending", item)
	fmt.Println("sendLendingitem", "nonce", item.AccountNonce)
	if err != nil {
		fmt.Println("rpcClient.Call FREx_sendLending failed", "err", err)
		os.Exit(1)
	}
}
