...
func VerifyBalance(isFREXLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenFREPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenFREPrice != nil && lendTokenFREPrice.Sign() > 0 {
					...
					defaultFeeInFRE := common.Big0
					if lendingToken.String() != common.FRENativeAddress {
						defaultFeeInFRE = new(big.Int).Mul(defaultFee, lendTokenFREPrice)
						defaultFeeInFRE = new(big.Int).Div(defaultFeeInFRE, lendingTokenDecimal)
					} else {
						defaultFeeInFRE = defaultFee
					}
					if defaultFeeInFRE.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isFREXLendingFork, Borrowing, lendTokenFREPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
