...
func TestGetSettleBalance(t *testing.T) {
	...
	lendTokenNotFRE := common.HexToAddress("0x0000000000000000000000000000000000000033")
	...
	type GetSettleBalanceArg struct {
		isFREXLendingFork      bool
		takerSide              string
		lendTokenFREPrice      *big.Int
	...
	tests := []struct {
		...
		{
			"LendToken is FRE, quantity too small, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
				...
				big.NewInt(150),
				big.NewInt(100), // 1%
				common.HexToAddress(common.FRENativeAddress),
				common.Address{},
				common.BasePrice,
				common.BasePrice,
				common.BasePrice,
		...
		{
			"LendToken is not FRE, quantity too small, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
		...
		{
			"LendToken is not FRE, no error, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
		...
		{
			"LendToken is not FRE, no error, taker INVEST",
			GetSettleBalanceArg{
				true,
				Investing,
				common.BasePrice,
		...
		{
			"LendToken is FRE, no error, taker invest",
			GetSettleBalanceArg{
				true,
				Investing,
				common.BasePrice,
			...
			&LendingSettleBalance{
				Taker: TradeResult{
					...
					InToken:  common.Address{},
					InTotal:  common.Big0,
					OutToken: common.HexToAddress(common.FRENativeAddress),
					OutTotal: lendQuantity,
				},
				Maker: TradeResult{
					Fee:      fee,
					InToken:  common.HexToAddress(common.FRENativeAddress),
					InTotal:  lendQuantityExcluded,
					OutToken: collateral,
					OutTotal: collateralLocked,
				},
		...
		{
			"LendToken is FRE, no error, taker Borrow",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := GetSettleBalance(tt.args.isFREXLendingFork, tt.args.takerSide, tt.args.lendTokenFREPrice, tt.args.collateralPrice, tt.args.depositRate, tt.args.borrowFeeRate, tt.args.lendingToken, tt.args.collateralToken, tt.args.lendTokenDecimal, tt.args.collateralTokenDecimal, tt.args.quantityToLend)
