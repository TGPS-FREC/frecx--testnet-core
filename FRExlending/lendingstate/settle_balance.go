...
const DefaultFeeRate = 100 // 100 / FREXBaseFee = 100 / 10000 = 1%
var (
...
func GetSettleBalance(isFREXLendingFork bool,
	takerSide string,
	lendTokenFREPrice,
	collateralPrice,
	depositRate,
	...
	if !isFREXLendingFork {
		if takerSide == Borrowing {
			...
			takerFee = new(big.Int).Div(takerFee, common.FREXBaseFee)
			...
			if lendingToken.String() != common.FRENativeAddress && lendTokenFREPrice != nil && lendTokenFREPrice.Cmp(common.Big0) > 0 {
				exTakerReceivedFee := new(big.Int).Mul(takerFee, lendTokenFREPrice)
				exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, lendTokenDecimal)

				defaultFeeInFRE := new(big.Int).Mul(defaultFee, lendTokenFREPrice)
				defaultFeeInFRE = new(big.Int).Div(defaultFeeInFRE, lendTokenDecimal)

				if (exTakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInFRE.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("takerFee too small", "quantityToLend", quantityToLend, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInFRE", defaultFeeInFRE)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken.String() == common.FRENativeAddress {
			...
			if lendingToken.String() != common.FRENativeAddress && lendTokenFREPrice != nil && lendTokenFREPrice.Cmp(common.Big0) > 0 {
				exMakerReceivedFee := new(big.Int).Mul(makerFee, lendTokenFREPrice)
				exMakerReceivedFee = new(big.Int).Div(exMakerReceivedFee, lendTokenDecimal)

				defaultFeeInFRE := new(big.Int).Mul(defaultFee, lendTokenFREPrice)
				defaultFeeInFRE = new(big.Int).Div(defaultFeeInFRE, lendTokenDecimal)

				if (exMakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInFRE.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("makerFee too small", "quantityToLend", quantityToLend, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInFRE", defaultFeeInFRE)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken.String() == common.FRENativeAddress {
		...
		borrowFee = new(big.Int).Div(borrowFee, common.FREXBaseFee)
		...
		if lendingToken.String() != common.FRENativeAddress && lendTokenFREPrice != nil && lendTokenFREPrice.Cmp(common.Big0) > 0 {
			...
			exReceivedFee := new(big.Int).Mul(borrowFee, lendTokenFREPrice)
			exReceivedFee = new(big.Int).Div(exReceivedFee, lendTokenDecimal)

			defaultFeeInFRE := new(big.Int).Mul(defaultFee, lendTokenFREPrice)
			defaultFeeInFRE = new(big.Int).Div(defaultFeeInFRE, lendTokenDecimal)

			if (exReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exReceivedFee.Sign() > 0) || defaultFeeInFRE.Cmp(common.RelayerLendingFee) <= 0 {
				log.Debug("takerFee too small", "quantityToLend", quantityToLend, "borrowFee", borrowFee, "exReceivedFee", exReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInFRE", defaultFeeInFRE)
				return result, ErrQuantityTradeTooSmall
			}
		} else if lendingToken.String() == common.FRENativeAddress {
