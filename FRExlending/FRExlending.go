...
func New(FREx *FREx.FREX) *Lending {
	...
	lending.StateCache = lendingstate.NewDatabase(FREx.GetLevelDB())
	lending.FREx = FREx
	return lending
}

func (l *Lending) GetLevelDB() FRExDAO.FREXDAO {
	return l.FREx.GetLevelDB()
}

func (l *Lending) GetMongoDB() FRExDAO.FREXDAO {
	return l.FREx.GetMongoDB()
}
	...
	return []rpc.API{
		{
			...
			Service:   NewPublicFREXLendingAPI(l),
			Public:    true,
		},
	}
}
...
func (l *Lending) GetLendingState(block *types.Block, author common.Address) (*lendingstate.LendingStateDB, error) {
	...
	if l.StateCache == nil {
		return nil, errors.New("Not initialized FREx")
	}
...
func (l *Lending) RollbackLendingData(txhash common.Hash) error {
	...
	if items != nil {
		for _, item := range items.([]*lendingstate.LendingItem) {
			c, ok := l.lendingItemHistory.Get(txhash)
			log.Debug("FRExlending reorg: rollback lendingItem", "txhash", txhash.Hex(), "item", lendingstate.ToJSON(item), "lendingItemHistory", c)
			if !ok {
				log.Debug("FRExlending reorg: remove item due to no lendingItemHistory", "item", lendingstate.ToJSON(item))
			...
			if (lendingItemHistory == lendingstate.LendingItemHistoryItem{}) {
				log.Debug("FRExlending reorg: remove item due to empty lendingItemHistory", "item", lendingstate.ToJSON(item))
				if err := db.DeleteObject(item.Hash, &lendingstate.LendingItem{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingItem. Err: %v . Item: %s", err.Error(), lendingstate.ToJSON(item))
				}
				continue
			...
			log.Debug("FRExlending reorg: update item to the last lendingItemHistory", "item", lendingstate.ToJSON(item), "lendingItemHistory", lendingItemHistory)
	...
	if items != nil {
		for _, trade := range items.([]*lendingstate.LendingTrade) {
			c, ok := l.lendingTradeHistory.Get(txhash)
			log.Debug("FRExlending reorg: rollback LendingTrade", "txhash", txhash.Hex(), "trade", lendingstate.ToJSON(trade), "LendingTradeHistory", c)
			if !ok {
				log.Debug("FRExlending reorg: remove trade due to no LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
			...
			if (lendingTradeHistoryItem == lendingstate.LendingTradeHistoryItem{}) {
				log.Debug("FRExlending reorg: remove trade due to empty LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
				if err := db.DeleteObject(trade.Hash, &lendingstate.LendingTrade{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingTrade. Err: %v . Trade: %s", err.Error(), lendingstate.ToJSON(trade))
				}
				continue
			...
			log.Debug("FRExlending reorg: update trade to the last lendingTradeHistoryItem", "trade", lendingstate.ToJSON(trade), "lendingTradeHistoryItem", lendingTradeHistoryItem)
			if err := db.PutObject(trade.Hash, trade); err != nil {
