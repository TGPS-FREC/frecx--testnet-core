...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		lendTokenFREPrice, collateralPrice, err := l.GetCollateralPrices(header, chain, statedb, tradingStateDb, collateralToken, order.LendingToken)
		if err != nil {
			return nil, nil, nil, err
		}
		if lendTokenFREPrice == nil || lendTokenFREPrice.Sign() <= 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenFREPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, header, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		isFREXLendingFork := chain.Config().IsTIPFREXLending(header.Number)
		settleBalanceResult, err := lendingstate.GetSettleBalance(isFREXLendingFork, takerOrder.Side, lendTokenFREPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.FREx.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
	...
	if !chain.Config().IsTIPFREXCancellationFee(header.Number) {
		...
		tokenCancelFee, tokenPriceInFRE = l.getCancelFee(chain, statedb, tradingStateDb, &originOrder, feeRate)
	}
...
func (l *Lending) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *lendingstate.LendingItem, feeRate *big.Int) (*big.Int, *big.Int) {
	...
	cancelFee, tokenPriceInFRE := common.Big0, common.Big0
	var err error
	if order.Side == lendingstate.Investing {
		cancelFee, tokenPriceInFRE, err = l.FREx.ConvertFREToToken(chain, statedb, tradingStateDb, order.LendingToken, common.RelayerLendingCancelFee)
	} else {
		cancelFee, tokenPriceInFRE, err = l.FREx.ConvertFREToToken(chain, statedb, tradingStateDb, order.CollateralToken, common.RelayerLendingCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, baseToken)
...
// - Have direct pair in FREX: lendToken/CollateralToken or CollateralToken/LendToken
// - Have pairs with FRE:
// -  lendToken/FRE and CollateralToken/FRE
// -  FRE/lendToken and FRE/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenFREPrice: price of ticker lendToken/FRE
	// collateralFREPrice: price of ticker collateralToken/FRE
	...
	collateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().S2PoS.Epoch == header.Number.Uint64()/chain.Config().S2PoS.Epoch

	lendTokenFREPrice, err := l.GetFREBasePrices(header, chain, statedb, tradingStateDb, lendingToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralPriceUpdatedFromContract {
		...
		return lendTokenFREPrice, collateralPriceFromContract, nil
	}
	lendingTokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, lendingToken)
	...
	inverseCollateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().S2PoS.Epoch == header.Number.Uint64()/chain.Config().S2PoS.Epoch
	if inverseCollateralPriceUpdatedFromContract {
		...
		return lendTokenFREPrice, collateralPrice, nil
	}
	// if contract doesn't provide any price information
	// getting price from pair in FREx
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in FREx", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
		return lendTokenFREPrice, lastAveragePrice, nil
	}
	collateralFREPrice, err := l.GetFREBasePrices(header, chain, statedb, tradingStateDb, collateralToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralFREPrice == nil || lendTokenFREPrice == nil {
		return common.Big0, common.Big0, nil
	}
	// Calculate collateral/LendToken price from collateral/FRE, lendToken/FRE
	collateralPrice = new(big.Int).Mul(collateralFREPrice, lendingTokenDecimal)
	collateralPrice = new(big.Int).Div(collateralPrice, lendTokenFREPrice)
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/FRE, lendToken/FRE", "collateralPrice", collateralPrice,
		"collateralFREPrice", collateralFREPrice, "lendingTokenDecimal", lendingTokenDecimal, "lendTokenFREPrice", lendTokenFREPrice)
	return lendTokenFREPrice, collateralPrice, nil
}

func (l *Lending) GetFREBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {

	tokenFREPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, token, common.HexToAddress(common.FRENativeAddress))
	tokenFREPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().S2PoS.Epoch == header.Number.Uint64()/chain.Config().S2PoS.Epoch

	if token == common.HexToAddress(common.FRENativeAddress) {
		return common.BasePrice, nil
	} else if tokenFREPriceUpdatedFromContract {
		// getting lendToken price from contract first
		// otherwise, getting from FREx lendToken/FRE
		log.Debug("Getting token/FRE price from contract", "price", tokenFREPriceFromContract)
		return tokenFREPriceFromContract, nil
	} else {
		FRETokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.HexToAddress(common.FRENativeAddress), token)
		FRETokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().S2PoS.Epoch == header.Number.Uint64()/chain.Config().S2PoS.Epoch
		if FRETokenPriceUpdatedFromContract && FRETokenPriceFromContract != nil && FRETokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from FREx lendToken/FRE
			log.Debug("Getting FRE/token from contract", "price", FRETokenPriceFromContract)
			tokenDecimal, err := l.FREx.GetTokenDecimal(chain, statedb, token)
			log.Debug("GetTokenDecimal", "token", token.Hex(), "err", err)
			if err != nil || tokenDecimal == nil || tokenDecimal.Sign() == 0 {
				return nil, err
			}
			tokenFREPrice := new(big.Int).Mul(common.BasePrice, tokenDecimal)
			tokenFREPrice = new(big.Int).Div(tokenFREPrice, FRETokenPriceFromContract)
			return tokenFREPrice, nil
		}
		tokenFREPrice, err := l.GetMediumTradePriceBeforeEpoch(chain, statedb, tradingStateDb, token, common.HexToAddress(common.FRENativeAddress))
		if err != nil {
			return nil, err
		}
		if tokenFREPrice != nil && tokenFREPrice.Sign() > 0 {
			log.Debug("Getting token/FRE from FREx", "price", tokenFREPrice, "err", err)
			return tokenFREPrice, nil
		}
	}
	log.Debug("Can't getting tokenFREPrice ", "token", token.Hex())
	return nil, nil
}
...
func (l *Lending) ProcessRepayLendingTrade(header *types.Header, chain consensus.ChainContext, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingstateDB *tradingstate.TradingStateDB, lendingBook common.Hash, lendingTradeId uint64) (trade *lendingstate.LendingTrade, err error) {
	...
	if tokenBalance.Cmp(paymentBalance) < 0 {
		...
		var err error
		if chain.Config().IsTIPFREXLending(header.Number) {
