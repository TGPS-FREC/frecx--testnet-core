...
// these are zero.
func (d *Downloader) Progress() FRECNET.SyncProgress {
...
func (d *Downloader) processFullSyncContent(height uint64) error {
	for {
		...
		if d.blockchain.Config() != nil && d.blockchain.Config().S2PoS != nil {
			epoch := d.blockchain.Config().S2PoS.Epoch
			gap := d.blockchain.Config().S2PoS.Gap
			inserts := []*fetchResult{}
