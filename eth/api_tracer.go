...
import (
	...
	"github.com/FRECNET/eth/tracers"
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	if err != nil {
		...
		for i := uint64(0); i < reexec; i++ {
			...
			if statedb, err = state.New(start.Root(), database); err == nil {
				FRExState, err = tradingstate.New(start.Root(), tradingstate.NewDatabase(api.eth.FREX.GetLevelDB()))
				if err == nil {
					break
				}
			}
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			_, _, _, err := api.eth.blockchain.Processor().Process(block, statedb, FRExState, vm.Config{}, feeCapacity)
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	statedb, FRExState, err := api.computeStateDB(parent, reexec)
	if err != nil {
		return nil, err
	}
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, FRExState, api.config, vm.Config{})
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, *tradingstate.TradingStateDB, error) {
	...
	FRExState := &tradingstate.TradingStateDB{}
	if err == nil {
		FRExState, err = api.eth.blockchain.OrderStateAt(block)
		if err == nil {
			return statedb, FRExState, nil
		}
	}
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		_, gas, err, tokenFeeUsed := core.ApplyTransaction(api.config, feeCapacity, api.eth.blockchain, nil, gp, statedb, FRExState, block.Header(), tx, usedGas, vm.Config{})
		if err != nil {
			return nil, vm.Context{}, nil, fmt.Errorf("tx %x failed: %v", tx.Hash(), err)
		}
