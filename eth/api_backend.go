...
import (
	...
	"github.com/FRECNET/core/bloombits"
...
func (b *EthApiBackend) GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, FRExState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error) {
	state.SetBalance(msg.From(), math.MaxBig256)
	vmError := func() error { return nil }

	context := core.NewEVMContext(msg, header, b.eth.BlockChain(), nil)
	return vm.NewEVM(context, state, FRExState, b.eth.chainConfig, vmCfg), vmError, nil
}
...
func (b *EthApiBackend) GetVotersRewards(masternodeAddr common.Address) map[common.Address]*big.Int {
	...
	engine := b.GetEngine().(*S2PoS.S2PoS)
	foundationWalletAddr := chain.Config().S2PoS.FoudationWalletAddr
	lastCheckpointNumber := number - (number % b.ChainConfig().S2PoS.Epoch) - b.ChainConfig().S2PoS.Epoch // calculate for 2 epochs ago
	...
	chainReward := new(big.Int).Mul(new(big.Int).SetUint64(chain.Config().S2PoS.Reward), new(big.Int).SetUint64(params.Ether))
	chainReward = util.RewardInflation(chain, chainReward, number, common.BlocksPerYear)
	signReward := util.RewardInflationSigner(chain, common.PercentPerEpoch, number, common.BlocksPerYear)
	fmt.Println("signReward::::", signReward)

	totalSigner := new(uint64)
	signers, err := contracts.GetRewardForCheckpoint(engine, chain, lastCheckpointBlock.Header(), rCheckpoint, totalSigner)

	fmt.Println("singers value at api_backend.go", signers)
	...
	rewardSigners, err := contracts.CalculateRewardForSigner(chainReward, signers, *totalSigner, state,signReward)
	fmt.Println("rewardSigners value at api_backend.go", rewardSigners)
...
func (b *EthApiBackend) GetEpochDuration() *big.Int {
	...
	block := chain.CurrentBlock()
	number := block.Number().Uint64()
	lastCheckpointNumber := number - (number % b.ChainConfig().S2PoS.Epoch)
	lastCheckpointBlockTime := chain.GetBlockByNumber(lastCheckpointNumber).Time()
	secondToLastCheckpointNumber := lastCheckpointNumber - b.ChainConfig().S2PoS.Epoch
...
func (b *EthApiBackend) GetOrderNonce(address common.Hash) (uint64, error) {
	FRExService := b.eth.GetFREX()
	if FRExService != nil {
		author, err := b.GetEngine().Author(b.CurrentBlock().Header())
		if err != nil {
			return 0, err
		}
		FRExState, err := FRExService.GetTradingState(b.CurrentBlock(), author)
		if err != nil {
			return 0, err
		}
		return FRExState.GetNonce(address), nil
	}
	return 0, errors.New("cannot find FREx service")
}

func (b *EthApiBackend) FRExService() *FREx.FREX {
	return b.eth.FREX
}

func (b *EthApiBackend) LendingService() *FRExlending.Lending {
