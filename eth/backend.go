...
import (
	...
	"github.com/FRECNET/eth/filters"
	"github.com/FRECNET/eth/hooks"
	...
	"github.com/FRECNET/internal/ethapi"
	"github.com/FRECNET/log"
	"github.com/FRECNET/miner"
...
func New(ctx *node.ServiceContext, config *Config, FREXServ *FREx.FREX, lendingServ *FRExlending.Lending) (*Ethereum, error) {
	...
	// Inject FREX Service into main Eth Service.
	if FREXServ != nil {
		eth.FREX = FREXServ
	}
	if lendingServ != nil {
	...
	if eth.chainConfig.S2PoS != nil {
		c := eth.engine.(*S2PoS.S2PoS)
		c.GetFREXService = func() utils.TradingService {
			return eth.FREX
		}
	...
	eth.blockchain, err = core.NewBlockChainEx(chainDb, FREXServ.GetLevelDB(), cacheConfig, eth.chainConfig, eth.engine, vmConfig)
	if err != nil {
		return nil, err
	}
	...
	if eth.chainConfig.S2PoS != nil {
		...
		signHook := func(block *types.Block) error {
			...
			// fix issue #228: block.NumberU64()%chainConfig.S2PoS.Epoch < common.MergeSignRange
			if block.NumberU64()%chainConfig.S2PoS.Epoch < common.MergeSignRange || block.NumberU64()%common.MergeSignRange == 0 || !eth.chainConfig.IsTIP2019(block.Number()) {
		...
		appendM2HeaderHook := func(block *types.Block) (*types.Block, bool, error) {
			...
			if m2 == eb {
				...
				header := block.Header()
				sighash, err := wallet.SignHash(accounts.Account{Address: eb}, S2PoS.SigHash(header).Bytes())
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db ethdb.Database) consensus.Engine {
	// If solid-state-proof-of-stake is requested, set it up
	if chainConfig.S2PoS != nil {
		return S2PoS.New(chainConfig.S2PoS, db)
	}
...
func (s *Ethereum) ValidateMasternode() (bool, error) {
	...
	if s.chainConfig.S2PoS != nil {
		//check if miner's wallet is in set of validators
		c := s.engine.(*S2PoS.S2PoS)
		...
		return false, fmt.Errorf("Only verify masternode permission in S2PoS protocol")
	}
	return true, nil
}
...
func (s *Ethereum) StartStaking(local bool) error {
	...
	if S2PoS, ok := s.engine.(*S2PoS.S2PoS); ok {
		wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
		if wallet == nil || err != nil {
			log.Error("Etherbase account unavailable locally", "err", err)
			return fmt.Errorf("signer missing: %v", err)
		}
		S2PoS.Authorize(eb, wallet.SignHash)
	}
	if local {
...
func (s *Ethereum) GetFREX() *FREx.FREX {
	return s.FREX
}
...
func (s *Ethereum) GetFREXLending() *FRExlending.Lending {
	return s.Lending
}
