...
//
// https://github.com/FRECNET/wiki/Management-APIs#personal_sign
...
//
// https://github.com/FRECNET/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) GetMasternodes(ctx context.Context, b *types.Block) ([]common.Address, error) {
	...
	if b.Number().Int64() >= 0 {
		...
		if engine, ok := s.b.GetEngine().(*S2PoS.S2PoS); ok {
			// Get block epoc latest.
			lastCheckpointNumber := prevBlockNumber - (prevBlockNumber % s.b.ChainConfig().S2PoS.Epoch)
			prevCheckpointBlock, _ := s.b.BlockByNumber(ctx, rpc.BlockNumber(lastCheckpointNumber))
			if prevCheckpointBlock != nil {
				masternodes = engine.GetMasternodesFromCheckpointHeader(prevCheckpointBlock.Header(), curBlockNumber, s.b.ChainConfig().S2PoS.Epoch)
			}
		} else {
			log.Error("Undefined S2PoS consensus engine")
		}
	}
...
func (s *PublicBlockChainAPI) GetCandidateStatus(ctx context.Context, coinbaseAddress common.Address, epoch rpc.EpochNumber) (map[string]interface{}, error) {
	...
	epochConfig := s.b.ChainConfig().S2PoS.Epoch
	...
	if engine, ok := s.b.GetEngine().(*S2PoS.S2PoS); ok {
		masternodes = engine.GetMasternodesFromCheckpointHeader(header, block.Number().Uint64(), s.b.ChainConfig().S2PoS.Epoch)
...
func (s *PublicBlockChainAPI) GetPreviousCheckpointFromEpoch(ctx context.Context, epochNum rpc.EpochNumber) (rpc.BlockNumber, rpc.EpochNumber) {
	...
	epoch := s.b.ChainConfig().S2PoS.Epoch
...
func (s *PublicBlockChainAPI) getCandidatesFromSmartContract() ([]utils.Masternode, error) {
	...
	validator, err := contractValidator.NewFREValidator(addr, client)
	if err != nil {
		return []utils.Masternode{}, err
	}
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config, timeout time.Duration) ([]byte, uint64, bool, error, error) {
	...
	FRExState, err := s.b.FRExService().GetTradingState(block, author)
	if err != nil {
		return nil, 0, false, err, nil
	}
	// Get a new instance of the EVM.
	evm, vmError, err := s.b.GetEVM(ctx, msg, statedb, FRExState, header, vmCfg)
	if err != nil {
		return nil, 0, false, err, nil
	}
...
func (s *PublicBlockChainAPI) findNearestSignedBlock(ctx context.Context, b *types.Block) *types.Block {
	...
	// Get block epoc latest
	checkpointNumber := signedBlockNumber - (signedBlockNumber % s.b.ChainConfig().S2PoS.Epoch)
...
func (s *PublicBlockChainAPI) findFinalityOfBlock(ctx context.Context, b *types.Block, masternodes []common.Address) (uint, error) {
	engine, _ := s.b.GetEngine().(*S2PoS.S2PoS)
...
*/
func (s *PublicBlockChainAPI) getSigners(ctx context.Context, block *types.Block, engine *S2PoS.S2PoS) ([]common.Address, error) {
	var err error
	...
	// Get block epoc latest.
	checkpointNumber := blockNumber - (blockNumber % s.b.ChainConfig().S2PoS.Epoch)
	checkpointBlock, _ := s.b.BlockByNumber(ctx, rpc.BlockNumber(checkpointNumber))

	masternodes := engine.GetMasternodesFromCheckpointHeader(checkpointBlock.Header(), blockNumber, s.b.ChainConfig().S2PoS.Epoch)
...
func (s *PublicBlockChainAPI) rpcOutputBlockSigners(b *types.Block, ctx context.Context, masternodes []common.Address) ([]common.Address, error) {
	...
	engine, ok := s.b.GetEngine().(*S2PoS.S2PoS)
	if !ok {
		log.Error("Undefined S2PoS consensus engine")
...
// PublicTransactionPoolAPI exposes methods for the RPC interface
type PublicFREXTransactionPoolAPI struct {
	b         Backend
	nonceLock *AddrLocker
}
...
// NewPublicTransactionPoolAPI creates a new RPC service with methods specific for the transaction pool.
func NewPublicFREXTransactionPoolAPI(b Backend, nonceLock *AddrLocker) *PublicFREXTransactionPoolAPI {
	return &PublicFREXTransactionPoolAPI{b, nonceLock}
}
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicFREXTransactionPoolAPI) SendOrderRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicFREXTransactionPoolAPI) SendLendingRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
...
func (s *PublicFREXTransactionPoolAPI) GetOrderTxMatchByHash(ctx context.Context, hash common.Hash) ([]*tradingstate.OrderItem, error) {
	var tx *types.Transaction
...
func (s *PublicFREXTransactionPoolAPI) GetOrderPoolContent(ctx context.Context) interface{} {
...
func (s *PublicFREXTransactionPoolAPI) GetOrderStats(ctx context.Context) interface{} {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicFREXTransactionPoolAPI) SendOrder(ctx context.Context, msg OrderMsg) (common.Hash, error) {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicFREXTransactionPoolAPI) SendLending(ctx context.Context, msg LendingMsg) (common.Hash, error) {
...
func (s *PublicFREXTransactionPoolAPI) GetOrderCount(ctx context.Context, addr common.Address) (*hexutil.Uint64, error) {
...
func (s *PublicFREXTransactionPoolAPI) GetBestBid(ctx context.Context, baseToken, quoteToken common.Address) (PriceVolume, error) {
	...
	FRExService := s.b.FRExService()
	if FRExService == nil {
		return result, errors.New("FREX service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return result, err
	}
	FRExState, err := FRExService.GetTradingState(block, author)
	if err != nil {
		return result, err
	}
	result.Price, result.Volume = FRExState.GetBestBidPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
...
func (s *PublicFREXTransactionPoolAPI) GetBestAsk(ctx context.Context, baseToken, quoteToken common.Address) (PriceVolume, error) {
	result := PriceVolume{}
	block := s.b.CurrentBlock()
	if block == nil {
		return result, errors.New("Current block not found")
	...
	result.Price, result.Volume = FRExState.GetBestAskPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if result.Price.Sign() == 0 {
...
func (s *PublicFREXTransactionPoolAPI) GetBidTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
	...
	FRExService := s.b.FRExService()
	if FRExService == nil {
		return nil, errors.New("FREX service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return nil, err
	...
	result, err := FRExState.DumpBidTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	price := FRExState.GetLastPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
...
func (s *PublicFREXTransactionPoolAPI) GetLastEpochPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	price := FRExState.GetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if price == nil || price.Sign() == 0 {
		return common.Big0, errors.New("Order book's price not found")
	}
	return price, nil
...
func (s *PublicFREXTransactionPoolAPI) GetCurrentEpochPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	price, _ := FRExState.GetMediumPriceAndTotalAmount(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if price == nil || price.Sign() == 0 {
		return common.Big0, errors.New("Order book's price not found")
	}
	return price, nil
...
func (s *PublicFREXTransactionPoolAPI) GetAskTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	result, err := FRExState.DumpAskTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetOrderById(ctx context.Context, baseToken, quoteToken common.Address, orderId uint64) (interface{}, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	orderitem := FRExState.GetOrder(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken), orderIdHash)
...
func (s *PublicFREXTransactionPoolAPI) GetTradingOrderBookInfo(ctx context.Context, baseToken, quoteToken common.Address) (*tradingstate.DumpOrderBookInfo, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	result, err := FRExState.DumpOrderBookInfo(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetLiquidationPriceTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpLendingBook, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	result, err := FRExState.DumpLiquidationPriceTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetInvestingTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	if lendingService == nil {
		return nil, errors.New("FREX Lending service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return nil, err
...
func (s *PublicFREXTransactionPoolAPI) GetBorrowingTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetLendingOrderBookInfo(tx context.Context, lendingToken common.Address, term uint64) (*lendingstate.DumpOrderBookInfo, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) getLendingOrderTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.LendingItem, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetLendingTradeTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.LendingTrade, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetLiquidationTimeTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetLendingOrderCount(ctx context.Context, addr common.Address) (*hexutil.Uint64, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetBestInvesting(ctx context.Context, lendingToken common.Address, term uint64) (InterestVolume, error) {
	...
	lendingService := s.b.LendingService()
	if lendingService == nil {
		return result, errors.New("FREX Lending service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return result, err
...
func (s *PublicFREXTransactionPoolAPI) GetBestBorrowing(ctx context.Context, lendingToken common.Address, term uint64) (InterestVolume, error) {
	result := InterestVolume{}
	block := s.b.CurrentBlock()
	if block == nil {
		return result, errors.New("Current block not found")
...
func (s *PublicFREXTransactionPoolAPI) GetBids(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	result, err := FRExState.GetBids(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetAsks(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
	...
	result, err := FRExState.GetAsks(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicFREXTransactionPoolAPI) GetInvests(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetBorrows(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("Current block not found")
	}
...
func (s *PublicFREXTransactionPoolAPI) GetLendingTxMatchByHash(ctx context.Context, hash common.Hash) ([]*lendingstate.LendingItem, error) {
	var tx *types.Transaction
	if tx, _, _, _ = core.GetTransaction(s.b.ChainDb(), hash); tx == nil {
		if tx = s.b.GetPoolTransaction(hash); tx == nil {
...
// GetLiquidatedTradesByTxHash returns trades which closed by FREX protocol at the tx of the give hash
func (s *PublicFREXTransactionPoolAPI) GetLiquidatedTradesByTxHash(ctx context.Context, hash common.Hash) (lendingstate.FinalizedResult, error) {
	var tx *types.Transaction
	if tx, _, _, _ = core.GetTransaction(s.b.ChainDb(), hash); tx == nil {
		if tx = s.b.GetPoolTransaction(hash); tx == nil {
...
func (s *PublicFREXTransactionPoolAPI) GetLendingOrderById(ctx context.Context, lendingToken common.Address, term uint64, orderId uint64) (lendingstate.LendingItem, error) {
	...
	lendingService := s.b.LendingService()
	if lendingService == nil {
		return lendingItem, errors.New("FREX Lending service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
...
func (s *PublicFREXTransactionPoolAPI) GetLendingTradeById(ctx context.Context, lendingToken common.Address, term uint64, tradeId uint64) (lendingstate.LendingTrade, error) {
...
func GetSignersFromBlocks(b Backend, blockNumber uint64, blockHash common.Hash, masternodes []common.Address) ([]common.Address, error) {
	...
	if engine, ok := b.GetEngine().(*S2PoS.S2PoS); ok {
...
func (s *PublicBlockChainAPI) GetStakerROI() float64 {
	...
	lastCheckpointNumber := blockNumber - (blockNumber % s.b.ChainConfig().S2PoS.Epoch) - s.b.ChainConfig().S2PoS.Epoch // calculate for 2 epochs ago
	...
	masternodeReward := new(big.Int).Mul(new(big.Int).SetUint64(s.b.ChainConfig().S2PoS.Reward), new(big.Int).SetUint64(params.Ether))
...
func (s *PublicBlockChainAPI) GetStakerROIMasternode(masternode common.Address) float64 {
	fmt.Println("GetStakerROIMasternode:::")
	...
	blockNumber := s.b.CurrentBlock().Number().Uint64()
	lastCheckpointNumber := blockNumber - blockNumber%s.b.ChainConfig().S2PoS.Epoch
	totalCap := new(big.Int).SetUint64(0)
