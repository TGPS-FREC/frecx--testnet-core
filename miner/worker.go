	...
	for {
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				if self.config.S2PoS != nil && self.config.S2PoS.Period == 0 {
					self.commitNewWork()
				}
			}
...
func (self *worker) wait() {
	for {
		...
		for result := range self.recv {
			...
			block := result.Block
			if self.config.S2PoS != nil && block.NumberU64() >= self.config.S2PoS.Epoch && len(block.Validator()) == 0 {
			...
			if work.config.S2PoS != nil {
				// epoch block
				if (block.NumberU64() % work.config.S2PoS.Epoch) == 0 {
					core.CheckpointCh <- 1
				}
			}
			...
			if self.config.S2PoS != nil {
				...
				// fix issue #228: block.NumberU64()%self.config.S2PoS.Epoch < common.MergeSignRange
				if block.NumberU64()%self.config.S2PoS.Epoch < common.MergeSignRange || block.NumberU64()%common.MergeSignRange == 0 || !self.config.IsTIP2019(block.Number()) {
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var FRExState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if self.config.S2PoS != nil {
		FREX := self.eth.GetFREX()
		FRExState, err = FREX.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get FREx state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := self.eth.GetFREXLending()
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.S2PoS != nil {
			...
			if !ok {
				...
				nearest := self.config.S2PoS.Epoch - (parent.Header().Number.Uint64() % self.config.S2PoS.Epoch)
				if uint64(h) >= nearest {
	...
	if self.config.S2PoS != nil && header.Number.Uint64()%self.config.S2PoS.Epoch != 0 {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.S2PoS != nil && self.chain.Config().IsTIPFREX(header.Number) {
			FREX := self.eth.GetFREX()
			FREXLending := self.eth.GetFREXLending()
			if FREX != nil && header.Number.Uint64() > self.config.S2PoS.Epoch {
				if header.Number.Uint64()%self.config.S2PoS.Epoch == 0 {
					err := FREX.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.S2PoS.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				//https://github.com/FRECNET-v1/pull/416
				if header.Number.Uint64()%self.config.S2PoS.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = FREX.ProcessOrderPending(header, self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = FREXLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.S2PoS.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = FREXLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.HexToAddress(common.FREXAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.FREXLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.FREXLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
		...
		FRExStateRoot := work.tradingState.IntermediateRoot()
		LendingStateRoot := work.lendingState.IntermediateRoot()
		txData := append(FRExStateRoot.Bytes(), LendingStateRoot.Bytes()...)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		// validate minFee slot for FREZ
		if tx.IsFREZApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateFREZApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("FREZApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		// validate balance slot, token decimal for FREX
		if tx.IsFREXApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateFREXApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("FREXApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		...
		if tx.To().Hex() == common.BlockSigners {
			...
			if blkNumber >= env.header.Number.Uint64() || blkNumber <= env.header.Number.Uint64()-env.config.S2PoS.Epoch*2 {
