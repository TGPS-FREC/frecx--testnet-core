...
import (
	...
	"github.com/FRECNET/contracts/randomize/contract"
)

type Randomize struct {
	*contract.FRERandomizeSession
...
func NewRandomize(transactOpts *bind.TransactOpts, contractAddr common.Address, contractBackend bind.ContractBackend) (*Randomize, error) {
	randomize, err := contract.NewFRERandomize(contractAddr, contractBackend)
	if err != nil {
		return nil, err
	}
...
func DeployRandomize(transactOpts *bind.TransactOpts, contractBackend bind.ContractBackend) (common.Address, *Randomize, error) {
	randomizeAddr, _, _, err := contract.DeployFRERandomize(transactOpts, contractBackend)
	if err != nil {
