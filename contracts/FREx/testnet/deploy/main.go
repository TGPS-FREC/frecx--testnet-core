...
import (
	...
	"github.com/FRECNET/contracts/FREx"
	simulation "github.com/FRECNET/contracts/FREx/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := FREx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	//init FREX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	FRExListtingAddr, FRExListing, err := FREx.DeployFREXListing(auth, client)
	if err != nil {
		log.Fatal("DeployFREXListing", err)
	}
	FRExListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> FREx listing address", FRExListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : FREx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := FREx.DeployRelayerRegistration(auth, client, FRExListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := FREx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, FRExListtingAddr)
	if err != nil {
	...
	applyFREXListing(FRExListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.FRENative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.FRENative)
	if err != nil {
		log.Fatal("Lending add base token FRE", err)
	}
	...
	relayerRegistration, err = FREx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.FRENative)
		}
	*/

	// FRE/BTC
	fromTokens = append(fromTokens, simulation.FRENative)
	...
	fmt.Println("wait 2s to apply token to list FREx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = FREx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// FRE 1 min
	baseTokens = append(baseTokens, simulation.FRENative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.TRC21TokenCap, simulation.BaseFRE)
		}
		if tokenName == "USDT" {
			d = 6
		...
		tokenAddr, _, err := FREx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *FREx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyFREXListing(FRExListing *FREx.FREXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		FRExListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		FRExListing.TransactOpts.Value = simulation.FREXListingFee
		_, err := FRExListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("FRExListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyFREXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := FREx.NewTRC21(auth, token["address"].(common.Address), client)
