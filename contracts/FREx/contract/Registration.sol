...
contract RelayerRegistration {
    ...
    address constant private FRENative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractFREXListing private FREXListing;
    ...
    constructor (address FRExListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        FREXListing = AbstractFREXListing(FRExListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with FRE(x/FRE)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 FRE is required for a deposit request");
    ...
    function indexOf(address[] memory FREPair, address target) internal pure returns (bool){
        for (uint i = 0; i < FREPair.length; i ++) {
            if (FREPair[i] == target) {
                return true;
            }
        }
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory FREPairs = new address[](fromTokens.length);
        address[] memory nonFREPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = FREXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == FRENative);
            b = b && (FREXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == FRENative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == FRENative) {
                FREPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == FRENative) {
                    FREPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonFREPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(FREPairs, nonFREPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory FREPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = FREXListing.getTokenStatus(toToken) || (toToken == FRENative);
        b = b && (FREXListing.getTokenStatus(fromToken) || fromToken == FRENative);
        if (!b) {
            return false;
        }

        if (fromToken == FRENative || toToken == FRENative) {
            return true;
        }

        // get tokens that paired with FRE
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == FRENative) {
                FREPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == FRENative) {
                    FREPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(FREPairs, toToken)) {
            return false;
        }
        return true;
    }
