...
func TestRewardBalance(t *testing.T) {
	contractBackend := backends.NewFRESimulatedBackend(core.GenesisAlloc{
	...
	}, 42000000, params.TestS2PoSMockChainConfig)
	...
	// validatorAddr, _, baseValidator, err := contract.DeployFREValidator(transactOpts, contractBackend, big.NewInt(50000), big.NewInt(99), big.NewInt(100), big.NewInt(100))
	validatorCap := new(big.Int)
	validatorCap.SetString("50000000000000000000000", 10)
	validatorAddr, _, baseValidator, err := contractValidator.DeployFREValidator(
		transactOpts,
		contractBackend,
		[]common.Address{addr},
...
func GetRewardBalancesRate(foudationWalletAddr common.Address, masterAddr common.Address, totalReward *big.Int, validator *contractValidator.FREValidator) (map[common.Address]*big.Int, error) {
...
func GetCandidatesOwnerBySigner(validator *contractValidator.FREValidator, signerAddr common.Address) common.Address {
	owner := signerAddr
	opts := new(bind.CallOpts)
...
func TestStatedbUtils(t *testing.T) {
	...
	contractBackend := backends.NewFRESimulatedBackend(genesisAlloc, 10000000, params.TestS2PoSMockChainConfig)
	transactOpts := bind.NewKeyedTransactor(key)
	...
	contractBackendForValidator := backends.NewFRESimulatedBackend(genesisAlloc, 10000000, params.TestS2PoSMockChainConfig)
