...
import (
	...
	"github.com/FRECNET/contracts/trc21issuer"
	"github.com/FRECNET/contracts/trc21issuer/simulation"
...
func airDropTokenToAccountNoFRE() {
...
func testTransferTRC21TokenWithAccountNoFRE() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have FRE
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoFRE()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoFRE()
		fmt.Println("Finish transfer trc21 token with a account no FRE")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no FRE")
	}
