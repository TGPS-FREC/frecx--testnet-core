...
var (
	...
	minApply = big.NewInt(0).Mul(big.NewInt(1000), big.NewInt(100000000000000000)) // 100 FRE
)
...
func TestFeeTxWithTRC21Token(t *testing.T) {
	...
	//  deploy a TRC21 SMC
	trc21TokenAddr, trc21, err := DeployTRC21(transactOpts, contractBackend, "TEST", "FRE", 18, cap, TRC21fee)
	if err != nil {
		t.Fatal("can't deploy smart contract: ", err)
	}
	contractBackend.Commit()
	...
	// airdrop token trc21 to a address no FRE
	...
	// access to address which received token trc21 but dont have FRE
