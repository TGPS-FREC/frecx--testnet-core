...
func GetM1M2(masternodes []common.Address, validators []int64, currentHeader *types.Header, config *params.ChainConfig) (map[common.Address]common.Address, uint64, error) {
	...
	if maxMNs > 0 {
		...
		if isForked {
			moveM2 = ((currentHeader.Number.Uint64() % config.S2PoS.Epoch) / uint64(maxMNs)) % uint64(maxMNs)
		}
...
// sigHash returns the hash which is used as input for the solid-state-proof-of-stake
