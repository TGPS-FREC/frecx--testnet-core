...
type SnapshotV1 struct {
	config   *params.S2PoSConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.S2PoSConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, signers []common.Address) *SnapshotV1 {
	snap := &SnapshotV1{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
func loadSnapshot(config *params.S2PoSConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*SnapshotV1, error) {
	blob, err := db.Get(append([]byte("S2PoS-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(SnapshotV1)
...
func (s *SnapshotV1) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("S2PoS-"), s.Hash[:]...), blob)
}
